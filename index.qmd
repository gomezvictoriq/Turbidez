...
### Leemos los archivos .csv por separado, para eso definimos dos DataFrame 
...
**Nota:** Se debió colocar la "r" delante de la direccion para que lea los archivos.
...
Para esto, realizamos un filtrado de creamos un nuevo DataFrame **"df_gis3x3"**, el cual se obtiene al hacer un filtrado en el DataFrame original. 
...
Lo defimos como **df_filtrado** , para esta estamos utilizamos la función *pd.merge* para realizar la combinación.
...
Si al guardamos con **index=False**, se omite y no se guarda en el csv.

#### Forma 2
Si decidimos poner **index=False** tenemos que usar una funación adicional antes de exportar, debido a que la turbidez está en el índice, no como una columna.

Por lo tanto luego de hacer el pivot se debe agregar la suguiente línea de código
...
## Regresion lineal con método de mínimos cuadrados
 
### Importo las funciones que me interesan y leamos el archivo csv creado previamente mis datos
