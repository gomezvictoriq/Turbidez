...
X_train, X_test, y_train, y_test = train_test_split(
  X, y, test_size=0.25, shuffle=True  #hace una mezcla 
)
```
...
## Prueba de correlación: turbidez vs bandas
...
df_correlaciones1.rename(columns={'Banda': 'Relación'}, inplace=True)

df_correlaciones1.to_csv('correlaciones_turbidez1.csv', index=False)

print(df_correlaciones1)
```
...
## Prueba de correlación: ln turbidez vs bandas
...
df_correlaciones2.rename(columns={'Banda': 'Relación'}, inplace=True)

df_correlaciones2.to_csv('correlaciones_turbidez2.csv', index=False)

print(df_correlaciones2)
```
...
## Prueba de correlación: ln turbidez vs ln bandas
...
df_correlaciones3['Banda'] = "ln_turb vs " + df_correlaciones3['Banda'].astype(str)
df_correlaciones3.rename(columns={'Banda': 'Relación'}, inplace=True)

df_correlaciones3.to_csv('correlaciones_turbidez3.csv', index=False)

print(df_correlaciones3)
```
...
descripcion = "Figura 3: Este gráfico de dispersión muestra la relación entre ln(B05) y ln(turbidez). Se observa una mejora de la linealidad entre ambas."
plt.figtext(0.5, -0.04, descripcion,
            wrap=True,                  # Permite que el texto se ajuste en varias líneas
            horizontalalignment='center', # Centra el texto horizontalmente
            fontsize=9,                 # Tamaño de la fuente
...
feature_names = X.columns # Obtiene los nombres de las columnas directamente de tu DataFrame X

equation_str = f"ln_turbidez = {intercept:.2f}"

for i, coef in enumerate(coefficients):
    equation_str += f" + ({coef:.2f} * {feature_names[i]})"

print("\n--- Ecuación del Modelo de Regresión Lineal Múltiple ---")
print(equation_str)
print("-------------------------------------------------------")

# (El resto de tu código para predicciones y evaluación, como ya lo tienes)
y_pred = regressor.predict(X_test)
...
for i, col_name in enumerate(X.columns):
    print(f"  {col_name}: {regressor.coef_[i]:.2f}")
print(f"Intercepto del modelo: {regressor.intercept_:.2f}")

plt.figure(figsize=(10, 6))
plt.scatter(y_test, y_pred, alpha=0.6)
plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', lw=2)
plt.xlabel("Valores Reales (ln_turbidez)")
