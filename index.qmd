...
Prueba con modelo Ramdon Forest
...
model = RandomForestRegressor(
    ...
    max_depth=None,        # profundidad ilimitada (se ajusta sola)
    max_features="sqrt",   # cantidad de variables por split
...
# --- Línea de tendencia sobre test ---
coef_linea = np.polyfit(y_test, y_test_pred, 1)  # pendiente, intercepto
x_tendencia = np.linspace(min_val, max_val, 100)  # CORREGIDO
y_tendencia = coef_linea[0] * x_tendencia + coef_linea[1]
plt.plot(x_tendencia, y_tendencia, '-', color='black',
         lw=2, label="Línea de tendencia (test)")
...
import os
from skimage.filters import threshold_otsu
import joblib  # para cargar modelo XGBoost si lo guardaste
...
# model = joblib.load("ruta_modelo_XGBoost.joblib")  # si lo guardaste previamente
...
for ruta in tifs:
    ...
    with rs.open(ruta) as raster:
        # Leer todas las bandas que usaste en entrenamiento
    ...
    # --- Crear array de features para XGBoost ---
    filas, cols = B03.shape
    X_img = np.stack([B01, B02, B03, B04, B05, B06, B07, B08, B11, B12, B8A], axis=-1)
    X_img_2d = X_img.reshape(-1, X_img.shape[-1])  # aplanar para el modelo

    # --- Predecir turbidez ---
    turbidez_pred = model.predict(X_img_2d)
